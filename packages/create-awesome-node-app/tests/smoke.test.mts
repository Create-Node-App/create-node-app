...
if (process.env.CNA_SKIP_GIT === "1" || process.env.SKIP_SMOKE === "1") {
  console.log(
    "[smoke] Skipping smoke test because CNA_SKIP_GIT or SKIP_SMOKE is set",
  );
  process.exit(0);
}
...
try {
  ...
  console.error(
    "Failed to build package before smoke test:",
    (e as Error).message,
  );
  process.exit(1);
}
...
try {
  ...
  const out = run(
    `node "${cliBin}" "${projectDir}" --no-install --template react-vite-boilerplate --no-interactive --verbose`,
    { cwd: packageRoot },
  );
  console.log(out);
